    ...
    <div class="modal-content">
      ...
      <form action="#" method="POST" id="edit_pekerjaan_form" enctype="multipart/form-data">
        ...
        <div class="modal-body p-4 bg-light">
            <div class="my-2">
              <label for="anggota">Nama Anggota</label>
                  <select name="user_anggota_id" id="user_anggota_id" class="form-control">
                      @php
                          $anggota = \App\Models\User::where('jabatan', 'anggota')->get();
                      @endphp
                      @foreach ($anggota as $p)
                          <option id="user_anggota_id" value="{{ $p->id }}">{{ $p->name }}</option>
                      @endforeach
                  </select>
            </div>
            <div class="my-2">
              <label for="Jenis_pekerjaan">Jenis Pekerjaan</label>
              <select id="user_anggota_id" name="jenis_pekerjaan" class="form-control">
                  <option value="disable" id="user_anggota_id">--Pilih Jenis Pekerjaan--</option>
                  <option value="Normal">Normal</option>
                  <option value="Sedang">Sedang</option>
                  <option value="Mendesak">Mendesak</option>
              </select>
    ...
    $(function() {
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        $.ajax({
          ...
          success: function(response) {
            $("#user_anggota_id").val(response.user_anggota_id);
